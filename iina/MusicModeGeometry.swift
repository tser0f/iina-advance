...
struct MusicModeGeometry: Equatable, CustomStringConvertible {
  ...
  func refit() -> MusicModeGeometry {
    ...
    Logger.log("Constraining miniPlayer. Video=\(isVideoVisible.yn) Playlist=\(isPlaylistVisible.yn) VideoAspect=\(videoAspectRatio.string6f), ReqSize=\(requestedSize), NewSize=\(newWindowSize)", level: .verbose)
